...
import { formatUsd, gapSentence } from "@/lib/format";
...
export default async function StockImage({
  params,
}: {
  ...
  return new ImageResponse(
    ...
    >
      ...
      >
        ...
        <span>
          {stock
            ? `${stock.reference.phrase.replace(/^./, (c) => c.toUpperCase())} vs onchain`
            : "after-hour.net"}
        </span>
      </div>
      ...
      >
        ...
        >
          {p?.logo ? (
            ...
            <span style={{ color: INK, fontSize: 44, fontWeight: 700 }}>
              {letters}
            </span>
          )}
        </div>
      ...
      {p && (
        ...
        >
          ...
          >
            {gapSentence(gap, stock?.reference.phrase ?? "Wall Street")}
          </div>
        </div>
      )}
