...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const priceLine =
    p.price != null
      ? `${formatUsd(p.price)} onchain, ${gapSentence(p.gapPct, stock.reference.phrase)}.`
      : "";
  const description =
    `${stock.name} (${stock.underlying}) as a tokenized stock on Solana. ${priceLine} ${stock.description} Buy from your own wallet, 24/7.`
      .replace(/\s+/g, " ")
      .trim();
...
const TRADABILITY_TIP: Record<string, string> = {
  ...
  stale:
    "The last trade was more than an hour ago. The price you see may not be where it would trade now.",
  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="flex items-start gap-3">
        <Link
          href="/stocks"
          className="icon-badge mt-1 h-8 w-8 shrink-0"
        ...
        <div className="min-w-0">
          ...
          <p className="text-muted text-sm">
            {stock.underlying} · {stock.sector} · {stock.tokens.length}{" "}
            {stock.tokens.length === 1 ? "issuer" : "issuers"} on Solana
          </p>
        ...
        <div className="flex flex-col gap-5 lg:order-1 lg:col-span-8">
          <div className="grid grid-cols-2 gap-3 sm:gap-4 xl:grid-cols-4">
            <StatCard
              ...
              detail={
                <>
                  <span className={`num font-medium ${gapTone(p.gapPct)}`}>
                    {gapSentence(p.gapPct, stock.reference.phrase)}
                  </span>
                </>
              }
            ...
            <StatCard
              ...
              detail={
                <span className="flex flex-wrap items-center gap-2">
                  <span className={`pill ${PILL[p.tradability]}`}>
                    {TRADABILITY_LABEL[p.tradability]}
                  </span>
            ...
            <StatCard
              ...
              value={
                stock.nextEarnings
                  ? earningsDate.format(
                      new Date(`${stock.nextEarnings.date}T12:00:00Z`),
                    )
                  : "–"
              }
              detail={
                stock.nextEarnings
                  ? stock.nextEarnings.timing === "unknown"
                    ? "time tbd"
                    : stock.nextEarnings.timing
                  : "none scheduled"
              }
          ...
          <PriceChart
            ...
            reference={p.reference}
            referenceLabel={stock.reference.short}
            now={now}
            symbol={p.symbol}
          />
          ...
          <section className="card p-5">
            ...
            <p className="text-muted mt-1 mb-3 text-sm">
              How far the onchain price sits from {stock.reference.phrase},
              every 15 minutes. Blue bars below the line: cheaper onchain. Red
              bars above: pricier.
            </p>
            <GapChart
              series={stock.gapSeries}
              referencePhrase={stock.reference.phrase}
            />
          </section>

          <GapHistory
            stats={stock.gapStats}
            symbol={p.symbol}
            referencePhrase={stock.reference.phrase}
          />
          ...
          <section className="card p-5">
            ...
            <p className="text-muted mt-1 text-sm">
              Same company, different wrappers. Each issuer has its own legal
              structure, so the tokens are not interchangeable. We quote the one
                  ...
                  {t.tradability === "none" ? (
                    <div className="mt-4 flex items-center justify-between gap-3">
                      <span className="text-muted text-sm">
                        No onchain price worth showing.
                      </span>
                      <Tip
                        text={TRADABILITY_TIP[t.tradability]}
                        underline={false}
                      >
                    ...
                    <div className="mt-4 grid grid-cols-[1fr_auto] items-center gap-x-3 gap-y-1">
                      <span className="num text-lg leading-tight font-semibold">
                        {formatUsd(t.price)}
                      </span>
                      <Tip
                        text={TRADABILITY_TIP[t.tradability]}
                      ...
                      <span className={`num text-xs ${gapTone(t.gapPct)}`}>
                        {gapWords(t.gapPct)}
                      </span>
                      <span className="text-muted justify-self-end text-xs">
                  ...
                  <li key={r.underlying}>
                    ...
                    >
                      ...
                      <span className="text-right">
                        ...
                        <span
                          className={`num block text-xs ${gapTone(r.gapPct)}`}
                        >
                          {gapWords(r.gapPct)}
                        </span>
                      </span>
