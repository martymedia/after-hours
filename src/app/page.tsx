...
export default async function OverviewPage() {
  ...
  const cheaper = [...withGap]
    .filter((r) => (r.gapPct ?? 0) < 0)
    .sort((a, b) => (a.gapPct ?? 0) - (b.gapPct ?? 0))
    .slice(0, 3);
  const pricier = [...withGap]
    .filter((r) => (r.gapPct ?? 0) > 0)
  ...
  const top =
    cheaper[0] ??
    [...withGap].sort(
      (a, b) => Math.abs(b.gapPct ?? 0) - Math.abs(a.gapPct ?? 0),
    )[0];
    ...
    <div className="flex flex-col gap-5">
      <div className="order-2 grid grid-cols-2 gap-3 sm:gap-4 lg:order-1 xl:grid-cols-4">
        <StatCard
          ...
          detail={
            closed
              ? `opens in ${formatDuration(new Date(data.phase.nextOpen).getTime() - now)}`
              : "regular session"
          }
          hint="Nasdaq and NYSE trade 9:30 AM to 4 PM New York time on weekdays. Outside that, tokenized stocks on Solana are the only place these shares change hands."
        />
        <StatCard
          index={1}
          icon={Activity}
        ...
        <StatCard
          ...
          icon={TrendingUp}
          label={
            top && (top.gapPct ?? 0) < 0 ? "Biggest discount" : "Biggest move"
          }
          ...
          badge={
            top && (
              <span
                className={`pill ${(top.gapPct ?? 0) < 0 ? "pill-blue" : "bg-soft-down text-down"}`}
              >
          ...
          detail={
            top ? (
              <span className={gapTone(top.gapPct)}>
                {gapSentence(top.gapPct, data.reference.phrase)}
              </span>
            ) : undefined
          }
        />
        <StatCard
          ...
          detail={
            nextEarnings
              ? `${earningsDate.format(new Date(`${nextEarnings.date}T12:00:00Z`))}, ${nextEarnings.timing === "unknown" ? "time tbd" : nextEarnings.timing}`
              : "none scheduled"
          }
        />
      </div>
      ...
      <div className="order-3 grid gap-5 lg:grid-cols-12">
        <section
          className="card rise flex flex-col p-5 lg:col-span-5"
          style={{ "--i": 5 } as React.CSSProperties}
        >
          <div className="mb-3 flex items-baseline justify-between">
            <h2 className="font-semibold">Where the gap is</h2>
            <span className="text-muted text-xs">
              vs {data.reference.phrase}
            </span>
          </div>
          <p className="text-blue mb-2 text-xs font-medium">
            Cheaper onchain than on Wall Street
          </p>
          <div className="grid flex-1 auto-rows-fr grid-cols-2 gap-3 sm:grid-cols-3">
            ...
            {cheaper.length === 0 && (
              <p className="text-muted col-span-full text-sm">
                Nothing trades at a discount right now.
              </p>
            )}
          </div>
          <p className="text-down mt-5 mb-2 text-xs font-medium">
            Pricier onchain than on Wall Street
          </p>
          <div className="grid flex-1 auto-rows-fr grid-cols-2 gap-3 sm:grid-cols-3">
            {pricier.map((m) => (
              <TrendCard key={m.underlying} row={m} />
            ...
            {pricier.length === 0 && (
              <p className="text-muted col-span-full text-sm">
                Nothing trades at a premium right now.
              </p>
            )}
          </div>
          <p className="text-muted mt-4 text-xs leading-relaxed">
            A discount is where a buy gets you the stock below the last real
            print. It can also mean the onchain market knows something; check
            the stock page before you act.
          </p>
        </section>

        <section
          className="card rise p-5 lg:col-span-7"
                  ...
                  <span className="w-28 shrink-0 text-right">
                    <span className="num block text-sm font-semibold">
                      {r.price == null ? (
                        ...
                        <CountUp value={r.price} from={0.9} kind="usd" />
                      )}
                    </span>
                    <span className={`num block text-xs ${gapTone(r.gapPct)}`}>
                      {gapWords(r.gapPct)}
                    </span>
                  </span>
                </Link>
