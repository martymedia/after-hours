...
export function TrendCard({ row }: { row: RadarRow }) {
  ...
  return (
    <Tilt
      className="flex"
      cardClassName="card-dark flex w-full overflow-hidden"
    >
      <Link
      ...
      >
        ...
        <div className="h-12">
          <Sparkline
            ...
            width={200}
            height={48}
            color={cheaper ? "var(--blue)" : "var(--down)"}
            fill
          />
        </div>
        <div>
          <div className="num text-xl font-semibold">
            {row.price == null ? (
              ...
              <CountUp value={row.price} from={0.9} kind="usd" />
            )}
          </div>
          <div
            className={`num text-xs ${cheaper ? "text-blue-light" : "text-down"}`}
          >
            {Math.abs(gap).toFixed(2)}% {cheaper ? "cheaper" : "pricier"} than
            Wall Street
          </div>
        </div>
      </Link>
