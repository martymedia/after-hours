...
export function WalletNavLink({ variant, expanded = true, pathname }: Props) {
  ...
  if (!connected) {
    ...
    return (
      <div className="rise mt-4 border-t border-line pt-4">
        {expanded && (
          <p className="text-muted-2 mb-2 pl-2 text-[11px] font-medium">
            Your wallet
          </p>
        )}
        {expanded ? (
  ...
  return (
    <div className="mt-4 border-t border-line pt-4">
      ...
      >
        <span
          className={`icon-badge relative h-10 w-10 shrink-0 ${active ? "bg-ink text-white border-ink" : ""}`}
        >
          <Wallet size={18} strokeWidth={1.75} />
          <span
            className="t-badge absolute -top-0.5 -right-0.5"
            data-open={dotOpen ? "true" : "false"}
            aria-hidden="true"
          >
        ...
        {expanded && (
          <span className="flex flex-col leading-tight">
            <span>Wallet</span>
            <span className="num text-muted-2 text-[11px] font-normal">
              {shortAddress(address)}
            </span>
          </span>
        )}
