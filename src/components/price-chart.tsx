...
function Chart({
  ...
  const x = (ts: number) =>
    PAD.left + ((ts - from) / (to - from)) * (W - PAD.left - PAD.right);
  const y = (v: number) =>
  ...
  for (let ts = from; ts <= to; ts += step) {
    ...
    if (range === "24h") {
      ...
      if (h === "0" || h === "00" || h === "24")
        ticks.push({ ts, label: dayLabel.format(d) });
    }
  }

  const linePts = visible.map(
    (c) => `${x(c.ts).toFixed(1)},${y(c.close).toFixed(1)}`,
  );
  ...
  function pick(target: SVGSVGElement, clientX: number) {
    ...
    for (const c of visible)
      if (Math.abs(c.ts - ts) < Math.abs(best.ts - ts)) best = c;
    setHover(best);
  }
  const onMove = (e: React.MouseEvent<SVGSVGElement>) =>
    pick(e.currentTarget, e.clientX);
    ...
    >
      ...
      {bands.map((b) => (
        <text
          ...
          fill="var(--blue-light)"
        >
          {x(b.end) - x(b.start) > 60 ? "Wall Street closed" : ""}
        </text>
      ))}
      {ticks.map((t) => (
        <g key={t.ts}>
      ...
      {hover && (
        <g>
          ...
          />
          <g
            transform={`translate(${hoverLeft ? x(hover.ts) - 154 : x(hover.ts) + 10}, ${PAD.top})`}
          >
