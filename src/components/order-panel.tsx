...
export function OrderPanel({ mint, symbol, reference, price, referencePhrase, disabled }: Props) {
  ...
  return (
    <section className="card-dark rise mt-5 p-5" style={{ "--i": 3 } as React.CSSProperties}>
      <div className="flex items-start justify-between gap-3">
        <div>
          <h2 className="flex items-center gap-2 font-semibold">
            <MoonStar size={16} strokeWidth={1.75} className="text-blue-light" />
        ...
        <Tip
          ...
          tone="light"
          underline={false}
          className="text-on-dark-muted shrink-0 text-xs"
        >
          How it works
        </Tip>
      </div>
      ...
      {step === "done" && placed ? (
        <div className="rise mt-4 rounded-2xl bg-white/10 p-4">
        ...
        <>
          <div className="mt-4 grid grid-cols-2 gap-3">
            <label className="rounded-2xl bg-white/10 p-3">
              <span className="text-on-dark-muted block text-xs">Spend</span>
              <span className="flex items-center gap-1">
                ...
                <input
                  ...
                  className="num w-full bg-transparent text-xl font-semibold text-white outline-none"
                  aria-label="USDC to spend"
                />
              </span>
            </label>
            <label className="rounded-2xl bg-white/10 p-3">
              <span className="text-on-dark-muted block text-xs">Buy at or below</span>
              <span className="flex items-center gap-1">
                <span className="text-on-dark-muted">$</span>
                <input
                  type="number"
          ...
          <div className="mt-2 flex flex-wrap items-center gap-1.5">
            {DISCOUNTS.map((d) => (
              <button
                ...
                className={`pill ${custom == null && discount === d ? "bg-white text-ink" : "bg-white/10 text-white hover:bg-white/20"}`}
              >
                {d}% under now
              </button>
            ))}
            <span className="text-on-dark-muted ml-auto text-xs">
              for{" "}
              <select value={days} onChange={(e) => setDays(Number(e.target.value))} className="rounded-full bg-white/10 px-2 py-0.5 text-white outline-none" aria-label="Order lifetime">
                {[1, 3, 7, 14, 30].map((d) => (
          ...
          <p className="text-on-dark-muted mt-3 text-sm">
            {target && shares ? (
              <>
                Buys about <span className="num text-white">{shares.toFixed(4)} {symbol}</span> if the onchain price reaches{" "}
                <span className="num text-white">{formatUsd(target)}</span>
          ...
          <div className="mt-4">
            {!ready ? (
              <span className="btn btn-white w-full opacity-50">Checking wallets…</span>
            ) : !connected ? (
              <ConnectButton label="Connect wallet to place the order" className="btn btn-white w-full" />
            ) : (
              <button type="button" className="btn btn-white w-full" disabled={busy || !target || usd < MIN_USD} onClick={place}>
                <SwapText
          ...
          {step === "error" && error && (
            <div key={error.title} className="rise t-shake mt-3 rounded-2xl bg-white/10 p-4">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-on-dark-muted mt-0.5 text-sm leading-relaxed">{error.hint}</p>
                </div>
              </div>
            </div>
          )}
