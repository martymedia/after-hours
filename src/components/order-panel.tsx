...
export function OrderPanel({ mint, symbol, side = "buy", held: heldProp, reference, price, referencePhrase, disabled }: Props) {
  ...
  const shares = sell ? (Number.isFinite(sharesIn) && sharesIn > 0 ? sharesIn : null) : target ? usd / target : null;
  const usdOut = sell && shares && target ? shares * target : usd;
  const vsRef = target && reference ? ((target - reference) / reference) * 100 : null;
  const vsNow = target && price ? ((target - price) / price) * 100 : null;
  const fillsNow = price != null && target != null && (sell ? target <= price : target >= price);
  const tooSmall = sell ? usdOut < MIN_USD : usd < MIN_USD;
  const overHeld = sell && held != null && shares != null && shares > held + 1e-9;
  ...
  async function place() {
    ...
    try {
      ...
      const res = await fetch("/api/order", {
        ...
        body: JSON.stringify({ mint, side, usd, shares, targetPrice: target, days: Number(days), userPublicKey: connected.account.address }),
      });
      const body = (await res.json()) as { transaction?: string; shares?: number; usd?: number; error?: string };
      ...
      setPlaced({ signature: sig, shares: body.shares ?? shares, target, usd: body.usd ?? usdOut });
      setStep("done");
    } catch (err) {
    ...
    return (
      <div className="rise mt-4 rounded-2xl bg-ink p-5 text-white">
        ...
        <p className="text-on-dark-muted mt-3 text-xs">
          The order account belongs to your wallet. Cancel any time from the Wallet page; {sell ? "unsold shares" : "unfilled USDC"} come straight back.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="mt-2 flex flex-col gap-5">
      {/* Amount */}
      <div>
        <p className="text-muted mb-1.5 text-xs font-medium">{sell ? "Sell" : "Spend"}</p>
        {sell ? (
          <>
            ...
            <label className="flex items-center gap-2 rounded-2xl bg-soft px-4 py-3">
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  if (Number.isFinite(v)) setUsd(Math.max(MIN_USD, Math.min(250000, v)));
                }}
      ...
      <div>
        <div className="mb-1.5 flex items-baseline justify-between">
          <p className="text-muted text-xs font-medium">{sell ? "Sell when the price is at or above" : "Buy when the price is at or below"}</p>
          <span className="text-muted num text-xs">now {formatUsd(price)}</span>
        </div>
        <label className="flex items-center gap-2 rounded-2xl bg-soft px-4 py-3">
          <span className="text-muted text-lg">$</span>
          <input
            type="text"
            inputMode="decimal"
            value={customText ?? (presetTarget != null ? presetTarget.toFixed(2) : "")}
      ...
      {/* Lifetime */}
      <div>
        <p className="text-muted mb-1.5 text-xs font-medium">Keep the order open for</p>
        <Seg ariaLabel="Order lifetime" value={days} onChange={setDays} options={DAYS} className="w-full justify-between" />
      </div>

      {/* Summary */}
      <div className={`rounded-2xl p-4 ${fillsNow || overHeld || tooSmall ? "bg-soft-warn" : "bg-ink text-white"}`}>
        {!target || !shares ? (
          <p className="text-muted text-sm">{sell ? "Enter how many shares to sell." : "Pick a price."}</p>
        ) : overHeld ? (
          <p className="text-warn text-sm">That is more than the {held?.toFixed(4)} {symbol} in this wallet.</p>
        ) : fillsNow ? (
          <p className="text-warn text-sm">
            {formatUsd(target)} is {sell ? "at or below" : "at or above"} the current price, so this would fill right away. Use {sell ? "Sell now" : "Buy"} instead, or pick a {sell ? "higher" : "lower"} price.
          </p>
        ) : tooSmall ? (
          <p className="text-warn text-sm">Orders need at least {formatUsd(MIN_USD, 0)} of value.</p>
        ) : (
          <>
            <p className="text-on-dark-muted text-xs">{sell ? "You get, if it fills" : "You get, if it fills"}</p>
            <p className="num mt-1 text-2xl font-semibold">{sell ? `about ${formatUsd(usdOut)}` : `about ${shares.toFixed(4)} ${symbol}`}</p>
            <p className="text-on-dark-muted num mt-1 text-sm">
              {sell ? `for ${shares.toFixed(4)} ${symbol} at ${formatUsd(target)}` : `at ${formatUsd(target)} per share`}
              {vsNow != null ? `, ${Math.abs(vsNow).toFixed(1)}% ${vsNow < 0 ? "under" : "over"} now` : ""}
              {vsRef != null ? `, ${Math.abs(vsRef).toFixed(1)}% ${vsRef < 0 ? "under" : "over"} ${referencePhrase}` : ""}.
            </p>
          </>
        )}
      </div>
      ...
      <div>
        {!ready ? (
          ...
          <button type="button" className="btn w-full" disabled={busy || !target || !shares || fillsNow || tooSmall || overHeld} onClick={place}>
            <SwapText
        ...
        <p className="text-muted mt-3 text-xs leading-relaxed">
          Your {sell ? "shares move" : "USDC moves"} into an order account that belongs to your wallet, not to us, and {sell ? "come" : "comes"} back if the order expires or you cancel.{" "}
          <Tip text="Jupiter Trigger, the same limit-order system as on jup.ag. Jupiter charges 0.1% on fills; After Hours adds nothing on orders.">Minimum {formatUsd(MIN_USD, 0)}.</Tip>
        </p>
      </div>
    </div>
  );
