...
import { armedAlerts, deleteWatchedOrder, firstTime, markAlertFired, subscribedOwners, upsertWatchedOrder, watchedOrders } from "./notify-db.ts";
...
type Token = { mint: string; symbol: string; name: string; underlying: string; decimals: number };
...
async function sweepOrders(owner: string, tokens: Map<string, Token>): Promise<void> {
  const active = (await listTriggerOrders(owner, "active")).map((o) => parse(o, tokens)).filter((x): x is NonNullable<typeof x> => Boolean(x && x.key));
  const known = new Map(watchedOrders(owner).map((w) => [w.order_key, w]));

  for (const o of active) {
    const prev = known.get(o.key);
    if (prev && o.remaining < prev.remaining - 0.01 && o.remaining > 0.01 && firstTime(`partial:${o.key}:${Math.round(o.remaining * 100)}`)) {
    ...
    upsertWatchedOrder({ order_key: o.key, owner, mint: o.token.mint, side: o.side, usd: o.usd, shares: o.shares, price: o.price, remaining: o.remaining });
    known.delete(o.key);
  }
  ...
  try {
    history = (await listTriggerOrders(owner, "history")).map((o) => parse(o, tokens));
  } catch {
    history = [];
  }
  for (const w of known.values()) {
    ...
    let title: string;
    let body: string;
    if (status.includes("ompl") || status.includes("fill") || (h && h.remaining <= 0.01)) {
      title = `${tokens.get(w.mint)?.symbol ?? "Order"} filled`;
      body = w.side === "buy" ? `You bought about ${w.shares.toFixed(4)} ${tokens.get(w.mint)?.symbol} at ${money(w.price)}.` : `You sold ${w.shares.toFixed(4)} ${tokens.get(w.mint)?.symbol} at ${money(w.price)}.`;
    ...
    if (firstTime(`closed:${w.order_key}`)) {
      await sendPush(owner, { title, body, url: `${SITE_URL}/wallet`, tag: `order-${w.order_key}` });
    }
...
async function sweepAlerts(tokens: Map<string, Token>): Promise<void> {
  ...
  for (const a of alerts) {
    ...
    await sendPush(a.owner, {
      ...
      body: `${t.name} trades at ${money(s.usd_price)} onchain against ${money(s.ref_price)}. Your alert was ${a.threshold}%.`,
